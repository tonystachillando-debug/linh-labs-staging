...
export const AgeOfAgents: React.FC = () => {
  ...
  const features = [
    { icon: <Swords />, label: t('aoa.feature1.label'), description: t('aoa.feature1.desc') },
    { icon: <Bot />, label: t('aoa.feature2.label'), description: t('aoa.feature2.desc') },
    { icon: <Link2 />, label: t('aoa.feature3.label'), description: t('aoa.feature3.desc') },
    { icon: <Globe />, label: t('aoa.feature4.label'), description: t('aoa.feature4.desc') },
  ];
      ...
      <div className="container mx-auto px-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
          <h2 className="font-display text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-6">
            {t('aoa.title')}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-300 via-cyan-400 to-blue-500">{t('aoa.titleHighlight')}</span>
          </h2>
          <p className="text-slate-400 text-lg md:text-xl max-w-3xl mx-auto font-light leading-relaxed">{t('aoa.subtitle')}</p>
        </motion.div>
        ...
        <div className="grid lg:grid-cols-2 gap-10 items-center mb-16">
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="group relative">
            ...
            <div className="relative rounded-2xl overflow-hidden border border-white/10 shadow-2xl shadow-emerald-900/20">
              <img src="/images/age-of-agents.png" alt="Age of Agents - RTS Game" className="w-full h-auto" />
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
              {stats.map((stat, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="text-center bg-slate-900/60 backdrop-blur-xl border border-white/5 rounded-xl p-3">
            ...
            <div className="space-y-4 mb-8">
              {features.map((feature, i) => (
                <motion.div key={i} initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.15 + i * 0.08 }} className="group/card flex items-start gap-4 p-4 bg-slate-900/50 backdrop-blur-xl border border-white/5 rounded-xl hover:border-emerald-500/20 transition-all duration-300">
            ...
            <a href="https://www.ageofagents.xyz/" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-3 bg-gradient-to-r from-emerald-500 to-cyan-600 text-white px-8 py-4 rounded-full font-bold text-sm uppercase tracking-widest hover:shadow-lg hover:shadow-emerald-500/25 transition-all duration-300 group">
              {t('aoa.cta')}
        ...
        <motion.div initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="flex flex-wrap justify-center gap-3">
          {['MegaETH', 'AI Agents', 'Real-Time Strategy', 'On-Chain', 'Multiplayer', 'Autonomous'].map((tag) => (
            <span key={tag} className="px-4 py-1.5 bg-slate-900/60 border border-white/5 rounded-full text-slate-500 text-xs font-medium">{tag}</span>
          ))}
        </motion.div>
      </div>
    </section>
